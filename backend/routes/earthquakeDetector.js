...
//  Polls USGS every 30 seconds, triggers allocation for M4.5+ events
//  SMS removed — uses smart resource allocator instead
// ============================================================

const { findNearestHospitals } = require("./hospitalService");
const { allocateBestTeam }     = require("./resourceAllocator");
...
async function processQuake(feature, io) {
  ...
  try {
    // ── Step 1: Find nearest hospitals ───────────────────────
    const hospitals = await findNearestHospitals(lat, lng);

    if (!hospitals.length) {
      console.warn(`[Earthquake] No hospitals found near (${lat}, ${lng}) — skipping allocation`);
      return;
    }

    console.log(`[Earthquake] Found ${hospitals.length} hospital(s) near ${place}`);

    // ── Step 2: Allocate best team for earthquake ────────────
    console.log(`[Earthquake] Running smart resource allocation...`);
    const allocation = allocateBestTeam(lat, lng, "earthquake");

    if (allocation) {
      console.log(`[Earthquake] ✅ Team allocated: "${allocation.team}"`);
      console.log(`[Earthquake]    ETA    : ${allocation.eta}`);
      console.log(`[Earthquake]    Reason : ${allocation.reason}`);
    } else {
      console.warn(`[Earthquake] ⚠️  No available teams for earthquake at ${place}`);
    }

    // ── Step 3: Emit to all WebSocket clients ────────────────
    if (io) {
...
async function pollEarthquakes(io) {
  try {
    ...
    // Process sequentially to avoid race conditions on state
